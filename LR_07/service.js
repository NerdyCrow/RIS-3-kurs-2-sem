...
if (process.argv.length < 3) throw Error("Service address was not specified!");

const service = config.services[process.argv[2]];

if (!service) throw Error("Invalid service address!");

const logger = log4js
  .configure(config.log)
  .getLogger(`Service-${service.address}`);
...
function setCoordinator() {
  ...
  fs.writeFile(
    "./coordinator.json",
    JSON.stringify({ address: service.address }, null, 2),
    (error) => {
      if (error) logger.error(error.toString());
      else
        socket.send("COORDINATOR", config.port, config.broadcast, (error) =>
          error ? logger.error(error.toString()) : null
        );
    }
  );
}
...
function startVoting() {
  ...
  self = true;
  setTimeout(() => {
    if (self) setCoordinator();
  }, config.coordinatorVotingTimeout);
  Object.values(config.services)
    .filter((s) => s.address > service.address)
    .forEach((s) => {
      socket.send("VOTING", config.port, s.address, (error) =>
        error ? logger.error(error.toString()) : null
      );
    });
}
  ...
  .on("message", (msg, rinfo) => {
    switch (msg.toString()) {
      case "TIME":
        ...
        break;
      case "CHECK":
        socket.send("OK", rinfo.port, rinfo.address, (error) =>
          error ? logger.error(error.toString()) : null
        );
        break;
      case "VOTING":
    ...
    fs.readFile("./coordinator.json", (error, data) => {
      if (error) logger.error(error.toString());
        ...
        if (coordinator.address !== service.address) {
          ...
          socket.send("CHECK", config.port, coordinator.address, (error) =>
            error ? logger.error(error.toString()) : null
          );
