    ...
    if (msg.toString() === "TIME") {
      fs.readFile("./coordinator.json", (error, data) => {
        if (error) logger.error(error.toString());
          ...
          logger.info(
            `Time request: client - ${rinfo.address}:${rinfo.port}, coordinator - ${coordinator.address}:${config.port}`
          );
          client = rinfo;
          service = coordinator;
          socket.send("TIME", config.port, coordinator.address, (error) =>
            error ? logger.error(error.toString()) : null
          );
        }
      });
    } else if (client) {
      socket.send(msg, client.port, client.address, (error) =>
        error ? logger.error(error.toString()) : null
      );
      client = null;
      service = null;
