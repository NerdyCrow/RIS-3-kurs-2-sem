...
queue.enter = () => {
  ...
  ca.status = "ENTER";
  socket.send(JSON.stringify(ca), ca.port, ca.ipAddress, (error) =>
...
let socket = udp
  ...
  .on("message", (msg, rinfo) => {
    ...
    switch (ca.status) {
      ...
      case "WAIT":
        if (
          !queue.find((e) => e.ipAddress === ca.ipAddress && e.port === ca.port)
        ) {
          queue.push(ca);
          console.log(`Wait critical section (${ca.ipAddress}:${ca.port})`);
          if (queue.at(0).status === "WAIT") queue.enter();
        } else console.warn("WARN: Repeating queue enter request:", ca);
        break;
      case "LEAVE":
        if (
          queue.at(0) &&
          queue.at(0).ipAddress === ca.ipAddress &&
          queue.at(0).port === ca.port
        ) {
          queue.leave();
          if (queue.at(0) && queue.at(0).status === "WAIT") queue.enter();
